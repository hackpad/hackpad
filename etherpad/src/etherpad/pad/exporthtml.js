...
function getPadHTMLDocument(pad, revNum, noDocType, removeTitleLine, unescapeCodeFragment) {
  var head = (noDocType?'':'<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" '+
    ...
    (noDocType?'':
      ...
      'ul.comment{list-style-image:url(\''+ 'https://' + appjet.config['etherpad.canonicalDomain'] +'/static/img/comment.png' +'\');} '+
      'ul.task{list-style-image:url(\''+ 'https://' + appjet.config['etherpad.canonicalDomain'] +'/static/img/unchecked.png' +'\');}'+
      'ul.taskdone{list-style-image:url(\''+ 'https://' + appjet.config['etherpad.canonicalDomain'] +'/static/img/checked.png' +'\');} '+
      '</style>'+
